...
class OCRService:
    """OCR service using Tesseract - free, local OCR. No API keys or credentials required."""
    ...
    def extract_text(self, image: bytes) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(pil_image, lang='pol+eng', config='--psm 6')
            return text.strip()
    ...
    def extract_text_from_roi(self, image_array: np.ndarray) -> str:
        ...
        try:
            ...
            pil_image = Image.fromarray(rgb_image)
            # Ensure image is RGB
            text = pytesseract.image_to_string(pil_image, lang='pol+eng', config='--psm 7')
            # Clean up the text
...
def check_tesseract():
    """Check if Tesseract is properly installed. Run this to diagnose OCR issues."""
    service = OCRService()
