"""Main OCR processing pipeline for ID card scanning. Uses Tesseract OCR - FREE, no API keys needed!"""
from typing import Dict, Any, Tuple, Optional
import numpy as np
from ocr.readzones import ZoneReader
from ocr.detectdocument import DocumentDetector
...
class IDCardOCRProcessor:
    ...
    # Field definitions
    FRONT_FIELDS = ['fnazwisko', 'fimiona', 'fobywatelstwo', 'fdataurodzenia', 'fplec', 'fnumerID', 'fdatawaznosci', 'fnumerkodu']
    BACK_FIELDS = ['bseriaid', 'bnumerid', 'bnumerident', 'bdatawydania', 'bktowydal', 'bimionarodzicow', 'bnazwiskorodowe', 'bmiejsceurodzenia', 'MRZ']

    def __init__(self, zones_dir: str = None, tesseract_path: str = None, target_width: int = 1000, target_height: int = 630):
        """Initialize IDCardOCRProcessor.
        Args:
            zones_dir: Directory containing zones.json files.
    ...
    def process_front(self, image_base64: str) -> Dict[str, str]:
        ...
        """
        return self.process_side(image_base64, 'front', self.FRONT_FIELDS)
    ...
    def process_back(self, image_base64: str) -> Dict[str, str]:
        ...
        Returns:
            Dictionary with extracted and validated field values.
        """
        return self.process_side(image_base64, 'back', self.BACK_FIELDS)

    def process_side(self, image_base64: str, side: str, fields: list) -> Dict[str, str]:
        ...
        if not detected:
            corrected = self.normalizer.normalize_document(image)  # Use original image if detection failed
